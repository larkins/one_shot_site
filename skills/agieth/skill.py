...
class AgiethClient:
    ...
    def setup_email_forwarding(
        ...
        delete_existing_drop_rule: bool = False,
    ) -> Dict:
        """One-shot setup for inbound email forwarding on a new domain.

        Deploys a per-domain Worker (email-forwarder-<domain>), generates a
        unique WEBHOOK_SECRET, sets the webhook URL and email domain, and
        creates the email routing rule — all in a single call.

        Use this for new domain setup instead of the lower-level building
        blocks (deploy_worker, set_cloudflare_worker_secret,
        create_email_routing_rule). One Worker per domain = clean secret
        isolation, no shared Worker state.

        Args:
            domain: Domain to enable forwarding for (e.g. "peristyle.ai")
            webhook_url: Inbound webhook URL (e.g. "https://mail.peristyle.ai/inbound")
            use_catch_all: If True, route ALL inbound mail; if False, only the
                specific addresses listed
            addresses: Specific email addresses to forward (used only when
                use_catch_all=False). Example: ["info@peristyle.ai", "michael@peristyle.ai"]
            webhook_secret: Optional. If omitted, a cryptographically random
                32-byte secret is auto-generated and returned in the response.
                The caller MUST propagate this secret to the receiving mail
                server (e.g. via the local-email skill on the mail server).
            delete_existing_drop_rule: Best-effort cleanup of any existing
                disabled catch-all drop rule before creating the new rule.
                Use this if use_catch_all=True fails with 2020 "Invalid rule
                operation".
        ...
        Returns:
            Dict with:
                ...
                - domain: "peristyle.ai"
                - zone_id: "..."
                - script_name: "email-forwarder-peristyle-ai"
                - webhook_url: "https://mail.peristyle.ai/inbound"
                - webhook_secret: "<auto-generated 32-byte secret>"
                - routing_rule: {the created rule}
                - instructions: Next steps for the caller (propagate secret to mail server)
        ...
        Example:
            ...
            >>> # Propagate secret to the mail server (e.g. via local-email skill)
        """
        payload: Dict = {
            "domain": domain,
            "webhook_url": webhook_url,
            "use_catch_all": use_catch_all,
            "addresses": addresses or [],
            "delete_existing_drop_rule": delete_existing_drop_rule,
        }
        if webhook_secret is not None:
            payload["webhook_secret"] = webhook_secret
        return self._post(
            "/api/v1/cloudflare/worker/setup-email-forwarding",
            data=payload,
        )
